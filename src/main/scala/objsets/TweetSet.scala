...
class Empty extends TweetSet {
  ...
  def mostRetweeted = throw new Exception("Empty.mostRetweeted")
  def descendingByRetweet = throw new Exception("Empty.descendingByRetweet")
  /**
   * The following methods are already implemented
   */
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def most(mostRe: Tweet, set: TweetSet): Tweet = {
      if(set.isEmpty) mostRe
      else {
        if(set.head.retweets>mostRe.retweets) most(set.head, set.tail)
        else most(mostRe, set.tail)
      }
    }
    
    most(this.head, this.tail)
  }
  
  def descendingByRetweet: TweetList = {
    def descend(ts: TweetSet,tl: TweetList): TweetList = {
      if(ts.isEmpty) tl
      else {
        val most = ts.mostRetweeted
        descend(ts.remove(most),tl + most)
      }
    }
    
    descend(this.remove(this.mostRetweeted), new Cons(this.mostRetweeted,Nil))
  }
  /**
   * The following methods are already implemented
   */
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = getTweetsFromListOfTopics(google,new Empty)
  lazy val appleTweets: TweetSet = getTweetsFromListOfTopics(apple,new Empty)
